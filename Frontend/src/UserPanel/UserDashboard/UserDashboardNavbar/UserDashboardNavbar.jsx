...
const UserDashboardNavbar = ({ onNavigate, activeTab }) => {
  const {user} = useUser();
  
  return (
    <div className="UserDashboardNavbar-container">
      {/* Profile Section */}
      <div className="UserDashboardNavbar-profile">
        <div className="UserDashboardNavbar-avatar">
          <FaUser />
        </div>
        <h2 className="UserDashboardNavbar-name">{user.firstName + " " + user.lastName}</h2>
      </div>
